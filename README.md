...
8. **Создание script.py:**
   - Добавлен скрипт для сбора информации о файлах проекта.
   - Скрипт собирает все файлы проекта, игнорируя определенные директории и файлы, и выводит их содержимое с указанием кодировки.

9. **Изменения в users/templates/users/profile_info.html:**
   - Добавлен закрывающий тег `{% endblock %}` для корректного завершения блока шаблона.

**Зачем нужны эти изменения:**

- Введение подписок на авторов и теги позволяет пользователям персонализировать свою ленту новостей, получая только интересующий их контент.
- Обновление шаблонов и представлений обеспечивает удобный интерфейс для управления подписками.
- Скрипт для сбора информации о файлах проекта помогает в анализе и отладке структуры проекта.
